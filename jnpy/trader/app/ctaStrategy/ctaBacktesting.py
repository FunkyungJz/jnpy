...
from jnpy.rpc import RpcClient, RpcServer, RemoteException
...
from jnpy.trader.vtGlobal import globalSetting
from jnpy.trader.vtObject import VtTickData, VtBarData
from jnpy.trader.vtConstant import *
from jnpy.trader.vtGateway import VtOrderData, VtTradeData
...
class BacktestingEngine(object):
    ...
    def crossLimitOrder(self):
        ...
        for orderID, order in workingLimitOrderDictCopy.items():       #fangyang 这里做了改写
...
class HistoryDataServer(RpcServer):
    ...
    def loadHistoryData(self, dbName, symbol, start, end):
        ...
        if end:  # 这里也改写 datetime 类型为 str
            flt = {'datetime':{'$gte':str(start.date()), '$lt':str(end.date())}}
        else:
            flt = {'datetime':{'$gte':str(start.date())}}
