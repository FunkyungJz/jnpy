...
def read_SHFE_hold(file, file_open_way):
    '''
...
if __name__ == '__main__':
    ...
    for ff in files_list:
        if '.csv' in ff:
            ...
            print(ff_with_path)
            try:
                df = read_SHFE_hold(ff_with_path, file_open_way='csv')
            except:
                df = read_SHFE_hold(ff_with_path, file_open_way='excel')
            finally:
                df_total = df_total.append(df, ignore_index=True)

    pd_mongo.write_df_json_to_db(df_total, database_name=database_name, collection_name=collection_name)
